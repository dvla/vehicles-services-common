...
class DocumentReferenceNumberSpec extends WordSpec with Matchers {
  ...
  "Document reference number validator" should {
    "indicate a valid reference number is valid" in {
      val valid = DocumentReferenceNumber.validate("1" * maxLength)
      valid should equal(true)
    }

    "indicate a truncated reference number is invalid" in {
      val valid = DocumentReferenceNumber.validate("1" * (minLength - 1))
      valid should equal(false)
    }

    "indicate a excessively long reference number is invalid" in {
      val valid = DocumentReferenceNumber.validate("1" * (maxLength + 1))
      valid should equal(false)
    }
